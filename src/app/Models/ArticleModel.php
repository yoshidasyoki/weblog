...
class ArticleModel extends DatabaseModel
{
    ...
    public function findUserArticles($userId)
    {
        $sql = <<<EOT
            SELECT article_id, username, title, sentence, articles.created_at, articles.updated_at, is_public
            From articles
            INNER JOIN users
                ON articles.user_id = users.user_id
    ...
    public function findUserArticle(int $articleId, int $userId)
    {
        $sql = <<<EOT
            SELECT article_id, title, sentence, is_public
            FROM articles
    ...
    public function updateArticle(int $articleId, int $userId, string $title, string $sentence, int $publicFlag)
    {
        $sql = <<<EOT
            UPDATE articles
            SET title = :title, sentence = :sentence, is_public = :is_public
            WHERE article_id = :article_id AND user_id = :user_id
        EOT;
